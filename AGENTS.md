...
- On Windows, generate local coverage reports with `powershell -ExecutionPolicy Bypass -File scripts/coverage-windows.ps1 -Scope tests -Format html` or `-Format json`; the helper uses `cargo-llvm-cov` with `CARGO_BUILD_JOBS=1` and an isolated target dir to avoid locked files and incomplete reports.
