...
`scripts/compare_public.py` 使用当前源码的 release 示例与本机 aria2c，逐次下载，避免两种工具同时争用带宽。每个连接档位默认重复三轮，第二轮交换执行顺序。
...
省略 `--url` 使用 Alpine 镜像源。`--rounds` 设置重复轮数，`--timeout` 设置单次进程总超时（默认 180 秒）。有可信发布方摘要时可传 `--sha256`。

实验配置：

- 1、8 个最大连接；1 MiB 分片和最小拆分设置；关闭文件预分配；各工具保留自身调度策略。
...
- bytehaul 开启任务日志和所有 tracing target 的 TRACE，并输出进度；aria2 文件和控制台均开启 DEBUG。没有新增依赖或修改下载库实现。
