...
`Cancelled`、`Paused`、`Completed` 是不同的结束状态。`cancel()` 和 `pause()` 都会结束当前任务；启用续传时，两者都会尝试保存可恢复状态。仍在等待并发名额的任务会直接结束而不启动，因此不会创建输出文件或断点文件。写盘或同步失败时保留此前的持久化断点。正常完成会尝试删除控制文件；单连接将清理失败视为错误，多连接的清理是尽力而为。

最终进度状态始终与 `wait()` 的返回值一致：只有整个任务成功（含配置的校验和检查）才是 `Completed`，其余为 `Cancelled`/`Paused`/`Failed`。停止任务报告的字节数是停止前已接收的部分；后续续传依赖的是持久化断点，而不是这个已接收字节数。
