...
origin 共享连接缓存；`MAXCONNECTS` 同时统计活动和空闲连接，不能直接设为空闲上限 k。
2026-09-19 公网复测暴露了固定 `MAXCONNECTS = 4` 在 8 路并发时几乎逐请求重建连接的问题。
现按 `k + max(活动请求数 - 1, 0)` 调整总缓存上限，为下一次完成后仍活跃的连接留出容量；
这不是活动连接数限制。k = 0 保持 fresh-connect + forbid-reuse。
同一次驱动可能完成多个请求，降低 MAXCONNECTS 又不会立即裁剪已有缓存，因此忙池允许暂时超出
空闲数量上限，仍执行原有空闲超时回收；当池完全空闲且估计容量超限时保守释放整个池。
缓存条目估计可能包含已被服务端关闭或 libcurl 淘汰的连接，保守释放可能少保留连接，
不会中断活动请求，也不会通过删除未知连接的时间记录来延后空闲回收。

回归测试 `eight_active_connections_reuse_with_four_idle_slots` 覆盖普通 DNS 和 CONNECT_TO：
7 条响应保持活跃，第 8 条连接连续执行 16 次请求，服务端累计只接受 8 条连接；
下载结束后验证空闲连接数量回到上限以内。
公网诊断轮 `20260919-133732` 使用同一 Citron ZIP、8 路、固定 1 MiB Range、multi_ip=true、TRACE；
修复前后实际 IP 均为 `104.25.244.197`，62 次请求的新建连接从 62 次降到 8 次，
耗时从 9.800 秒降到 4.460 秒，同轮 aria2 为 3.671 秒，三份文件 SHA-256 和 ZIP CRC 均通过。
运行目录 `target/public-compare/20260919-133732/` 中 `bytehaul` 是保留的修复前二进制，
`bytehaul-curl` 是修复后构建；`metadata.json` 保存二进制 SHA-256，`split_ips.csv` 保存分片 IP/连接 ID。
这是单轮公网诊断结果，不排除线路与服务端波动，也不代表已解决慢 IP 或完成多候选收敛验收。
