...
- 已实现 `Downloader(connect_timeout=...)` 包装，Python 构造函数直接接收 builder 参数
- 已实现 `DownloadTask` 包装 Rust 侧 `DownloadHandle`，提供 `wait()`、`cancel()`、`progress()` 方法
- `DownloadTask` 内部使用 `Arc<Mutex<Option<DownloadHandle>>>` 管理句柄生命周期，`wait()` 消费句柄，防止后台任务泄漏
- 已实现 `ProgressSnapshot` 冻结对象，包含 `total_size`、`downloaded`、`state`、`speed`、`elapsed_secs` 属性
- `DownloadState` 枚举映射为 Python 字符串常量（`"pending"`、`"downloading"` 等）
- `Instant` 已在绑定层转换为 `elapsed_secs: float`
- 阻塞型 `wait()` 调用期间通过 `allow_threads` 释放 GIL
- 已通过 `maturin develop` 完成本地安装和导入验证

### M3 ✅ 已完成

- 增加 Python 侧导入、参数校验与端到端测试
- 明确本地开发命令和构建命令
- 补充安装说明与示例
...
- 新增 `tests/test_bytehaul.py`，包含 26 个测试用例，涵盖：
  - 导入与版本验证（`TestImports`）
  - 异常层级检查
  - 参数校验错误（`TestConfigValidation`，7 个 case）
  - `download()` 便捷 API 端到端测试（`TestDownloadFunction`，4 个 case）
  - `Downloader` / `DownloadTask` 对象 API 测试（`TestDownloaderAPI`，12 个 case），包括 progress 轮询、cancel、wait 重复调用保护、404 异常等
- 测试使用内嵌 `http.server` 提供本地 HTTP 服务，无外部依赖
- 修复了 `Downloader.download()` 缺少 tokio runtime context 的问题（使用 `runtime.enter()` 进入 runtime 上下文）
- 开发依赖新增 `pytest>=8.0`，通过 `uv sync` 管理
- 本地开发命令：
  - 构建安装：`cd bindings/python && uv run maturin develop`
  - 运行测试：`cd bindings/python && uv run pytest tests/ -v`
...
- `bytehaul-python` crate 版本与根 `bytehaul` crate 保持同步（当前为 `0.1.3`），通过 `bytehaul = { path = "../..", version = "0.1.3" }` 约束
- `pyproject.toml` 补齐发布元数据：`readme`、`classifiers`（Alpha 状态、Python 3.9–3.13、Rust）、`pytest` 配置
- 使用 `abi3-py39` 特性，单平台只需构建一个 wheel 即可覆盖 Python 3.9+
- 新增 `bindings/python/README.md`，包含：
  - 安装说明（源码开发 / wheel 构建）
  - 完整 API 参考（`download()`、`Downloader`、`DownloadTask`、`ProgressSnapshot`、全部下载选项）
  - 用法示例（简单下载、带选项下载、对象 API、错误处理）
  - 测试运行命令
  - 跨平台 wheel 构建指引（maturin-action）
