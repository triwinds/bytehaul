...
## 0.2.1 实现摘要（2026-09-06）

从“校准后的结论”起的全部基线分析、原计划和 M1–M7 进展均保留为历史记录，其中“当前”指记录当时的实现，不代表 0.2.1。现行行为以本节、[架构说明](architecture.zh-CN.md) 和 [0.2.1 发布说明](https://github.com/triwinds/bytehaul/releases/tag/v0.2.1) 为准。

- 调度器详细状态改为仅保存已触及且未完成的 piece；活动 lease 与缺失区间使用增量计数，完成 piece 释放详细状态。dirty/inflight 提示从稀疏状态生成，继续兼容 V1/V2。
- 写缓存按 lease 接受连续追加，删除任意乱序、重叠区间合并能力；迟到 lease 数据仍由 writer 拒绝。全量 flush 按文件 offset 排序。
- 数据块大小与 writer 刷盘水位共同约束，保证小预算下能释放 permits；限速、预算和 channel 等待响应暂停及取消。
- 多连接 checkpoint 先截取完成状态，再等待 writer flush/sync，最后保存原快照，避免将同步之后完成的数据记为可恢复。
- 移除重复的 DNS 响应缓存和预解析，使用 Hickory 自带的有界 TTL 缓存。
- idle pool 继续使用现有显式开关；本地对比结果记录于任务研究材料，不能替代公网、TLS 和代理验证。
